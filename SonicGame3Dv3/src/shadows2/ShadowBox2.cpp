...
Matrix4f ShadowBox2::calculateCameraRotationMatrix()
{
	Matrix4f rotation;
	Vector3f temp(0, 1, 0);
	rotation.rotate(toRadians(-cam->getYaw()), &temp);
	temp.set(1, 0, 0);
	rotation.rotate(toRadians(-cam->getPitch()), &temp);
	return rotation;
}
...
void ShadowBox2::calculateWidthsAndHeights()
{
	...
	farWidth = SHADOW_DISTANCE * tanf(toRadians(VFOV));
	nearWidth = NEAR_PLANE * tanf(toRadians(VFOV));
